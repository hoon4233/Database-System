...
void log_buff_write(int64_t trx_prev_LSN, int trx_id, int type, int table_id,pagenum_t page_NUM, int index, char * old_img, char * new_img){
    ...
    //set next_undo point
        ...
        tmp_ret = log_buff_search_record(log_file, trx_prev_LSN, &prev_log );
        ...
        if(prev_log.BCR.type == LOG_UPD_TYPE){
            ...
            tmp_ret = log_buff_search_record(log_file, prev_log.BCR.prev_LSN, &pre_prev_log );
        ...
        if(prev_log.BCR.type == LOG_COM_TYPE){
            ...
            tmp_ret = log_buff_search_record(log_file, prev_log.COM.next_undo_LSN, &tmp_next_undo);
            ...
            tmp_ret = log_buff_search_record(log_file, tmp_next_undo.COM.prev_LSN, &tar_log);
