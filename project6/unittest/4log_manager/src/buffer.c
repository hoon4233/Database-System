...
int init_log_buff(int num_buf, char * log_path){
    ...
    log_buff->blocks = (log_record_t *) malloc(sizeof(log_record_t) * (num_buf+LOG_READ_ONLY));
    ...
    int file_exist = -1;
    file_exist = log_file_open(&log_file, log_path, &last_record);
    ...
    for(int i=0; i<num_buf+LOG_READ_ONLY; i++){
        log_buff->blocks[i].COM.log_size = -1;
        log_buff->blocks[i].COM.LSN = 0;
    ...
    log_buff->blocks[num_buf].BCR.type = 0; //read only buff
...
void log_buff_search_record(int file, int tar_LSN, const log_record_t* dest){
    for(int i = 0; i < log_buff->used_cnt; i++){
    ...
    log_file_read_record(file, tar_LSN, 0, dest);
}
...
void log_buff_write(int64_t trx_prev_LSN, int trx_id, int type, int table_id,pagenum_t page_NUM, int index, char * old_img, char * new_img){
    ...
    //set next_undo point
        ...
        log_buff_search_record(log_file, trx_prev_LSN, &prev_log );
        ...
        if(prev_log.BCR.type == LOG_UPD_TYPE){
            ...
            log_buff_search_record(log_file, prev_log.BCR.prev_LSN, &pre_prev_log );
        ...
        if(prev_log.BCR.type == LOG_COM_TYPE){
            ...
            log_buff_search_record(log_file, prev_log.COM.next_undo_LSN, &tmp_next_undo);
            ...
            log_buff_search_record(log_file, tmp_next_undo.COM.prev_LSN, &tar_log);
...
void log_buff_read_all_file(void){
    ...
    while(ret != -1){
        ...
        if(ret == -2){
            printf("log_buff_read_all_file end\n");
            return;
        }
