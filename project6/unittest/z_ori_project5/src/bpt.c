...
lock_t*
lock_acquire(int table_id, int64_t key, int trx_id, int lock_mode, char * value)
{
	pthread_mutex_lock(&trx_table_latch);
	pthread_mutex_lock(&lock_table_latch);
	lock_t * obj = NULL;
...
int trx_abort(int trx_id){
    ...
    if(tmp_ptr != NULL){  //obj를 가지고 있다면
        ...
        while(tmp_ptr->trxprevlock != NULL){ //전부 free 해주기
            ...
            //roll_back start
            if(free_obj->mode == 1){
                buff_update_read_page(free_obj->sentinel->tid, free_obj->sentinel->rid,&tmp_page);
                for( int i = 0; i < tmp_page.L.header.numKeys; i++){
                    if(tmp_page.L.records[i].key == free_obj->sentinel->rid){
                        memcpy(tmp_page.L.records[i].value, free_obj->old_v ,VALUE_SIZE);
                    }
                }
                buff_update_write_page(free_obj->sentinel->tid, free_obj->sentinel->rid,&tmp_page);
            }
            //roll_back end
            tmp_ptr = tmp_ptr->trxprevlock;
			ret = lock_release(free_obj);
        ...
        if(tmp_ptr->mode == 1){
            page_t tmp_page;
            buff_update_read_page(tmp_ptr->sentinel->tid, tmp_ptr->sentinel->rid,&tmp_page);
            for( int i = 0; i < tmp_page.L.header.numKeys; i++){
                if(tmp_page.L.records[i].key == tmp_ptr->sentinel->rid){
                    memcpy(tmp_page.L.records[i].value, tmp_ptr->old_v ,VALUE_SIZE);
                }
            }
            buff_update_write_page(tmp_ptr->sentinel->tid, tmp_ptr->sentinel->rid,&tmp_page);
        }
        //마지막 obj에 대해 roll back end
        tar_trx->first_obj = NULL;
		ret = lock_release(tmp_ptr);
